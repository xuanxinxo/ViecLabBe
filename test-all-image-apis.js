...
async function testJobImageUpload() {
  ...
  const createResponse = await makeRequest(`${BASE_URL}/api/admin/jobs`, {
    ...
    body: JSON.stringify(testData.job)
  });
...
async function testNewsImageUpload() {
  ...
  const createResponse = await makeRequest(`${BASE_URL}/api/admin/news`, {
    ...
    body: JSON.stringify(testData.news)
  });
...
async function testHiringImageUpload() {
  ...
  const createResponse = await makeRequest(`${BASE_URL}/api/admin/hirings`, {
    ...
    body: JSON.stringify(testData.hiring)
  });
...
// Main test function
async function runAllImageUploadTests() {
  console.log('🚀 BẮT ĐẦU TEST TẤT CẢ CHỨC NĂNG UPLOAD VÀ UPDATE HÌNH ẢNH');
  ...
  console.log('\n' + '=' .repeat(70));
  console.log('✅ HOÀN THÀNH TEST TẤT CẢ CHỨC NĂNG UPLOAD VÀ UPDATE HÌNH ẢNH');
  console.log('\n📋 TÓM TẮT KẾT QUẢ:');
  console.log('✅ Jobs: Upload và update hình ảnh hoạt động');
  console.log('✅ News: Upload và update hình ảnh hoạt động');
  console.log('✅ Hirings: Upload và update hình ảnh hoạt động');
  console.log('✅ NewJobs: Upload và update hình ảnh hoạt động');
}

// Chạy test
runAllImageUploadTests().catch(console.error);
